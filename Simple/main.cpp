...
void marker_run(int argc,char** argv){
    ...
    // declarations and instantiation of component
    SRef<SolARCameraOpencv> camera = xpcf::utils::make_shared<SolARCameraOpencv>();
    SRef<SolARMarker2DSquaredBinaryOpencv> binaryMarker = xpcf::utils::make_shared<SolARMarker2DSquaredBinaryOpencv>();
    SRef<SolARImageViewerOpencv> imageViewer = xpcf::utils::make_shared<SolARImageViewerOpencv>();
    SRef<SolARImageViewerOpencv> imageViewerGrey = xpcf::utils::make_shared<SolARImageViewerOpencv>();
    SRef<SolARImageViewerOpencv> imageViewerBinary = xpcf::utils::make_shared<SolARImageViewerOpencv>();
    SRef<SolARImageViewerOpencv> imageViewerContours = xpcf::utils::make_shared<SolARImageViewerOpencv>();
    SRef<SolARImageViewerOpencv> imageViewerFilteredContours = xpcf::utils::make_shared<SolARImageViewerOpencv>();
    SRef<SolARImageFilterOpencv> imageFilter = xpcf::utils::make_shared<SolARImageFilterOpencv>();
    SRef<SolARImageConvertorOpencv> imageConvertor = xpcf::utils::make_shared<SolARImageConvertorOpencv>();
    SRef<SolARContoursExtractorOpencv> contoursExtractor = xpcf::utils::make_shared<SolARContoursExtractorOpencv>();
    SRef<SolARContoursFilterBinaryMarkerOpencv> contoursFilter = xpcf::utils::make_shared<SolARContoursFilterBinaryMarkerOpencv>();
    SRef<SolARPerspectiveControllerOpencv> perspectiveController = xpcf::utils::make_shared<SolARPerspectiveControllerOpencv>();
    SRef<SolARDescriptorsExtractorSBPatternOpencv> patternDescriptorExtractor = xpcf::utils::make_shared<SolARDescriptorsExtractorSBPatternOpencv>();
    SRef<SolARDescriptorMatcherRadiusOpencv> patternMatcher = xpcf::utils::make_shared<SolARDescriptorMatcherRadiusOpencv>();
    SRef<SolARSBPatternReIndexer> patternReIndexer = xpcf::utils::make_shared<SolARSBPatternReIndexer>();
    SRef<SolARImage2WorldMapper4Marker2D> img2worldMapper = xpcf::utils::make_shared<SolARImage2WorldMapper4Marker2D>();
    SRef<SolARPoseEstimationOpencv> PnP = xpcf::utils::make_shared<SolARPoseEstimationOpencv>();
    SRef<SolAR3DOverlayOpencv> overlay3D = xpcf::utils::make_shared<SolAR3DOverlayOpencv>();
    ...
    while (process){
       ...
       if (patternDescriptorExtractor->extract(patches, filtered_contours, recognizedPatternsDescriptors, recognizedContours) != FrameworkReturnCode::_ERROR_)
       {
           ...
           std::cout << std::endl;
            ...
            if (patternMatcher->match(markerPatternDescriptor, recognizedPatternsDescriptors, patternMatches) == api::features::DescriptorMatcher::DESCRIPTORS_MATCHER_OK)
            {
#ifdef DEBUG
